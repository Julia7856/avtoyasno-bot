...
def get_calc_menu():
    ...
    markup.add('🌨️ Сезонные советы', '⬅️ Назад')
    return markup
...
def brands(message):
    ...
    markup.add('Changan', 'BAIC', '⬅️ Назад')
...
def text_handler(message):
    ...
    if 'стук' in text:
        bot.send_message(message.chat.id, ' Стук: гидрокомпенсаторы, ГРМ, подшипники. Нужна диагностика!')
    elif 'не заводится' in text:
        bot.send_message(message.chat.id, '🔍 Не заводится: АКБ, стартер, свечи, бензонасос.')
    elif 'check' in text or 'чек' in text:
        bot.send_message(message.chat.id, '🔍 Check Engine: считайте код сканером OBD2 и напишите мне!')
    else:
        bot.send_message(message.chat.id, '🤔 Не понял. Выберите пункт из меню.')
