...
def search(message):
    bot.send_message(message.chat.id, ' Напишите код ошибки (P0420) или проблему (стук, не заводится).', reply_markup=get_main_menu())
...
def calc_fuel_process(message):
    try:
        ...
        text = ' РЕЗУЛЬТАТ:\n\nРеальный расход: ' + str(round(consumption, 1)) + ' л/100км\n💰 Стоимость 1 км: ' + str(round(cost_per_km, 2)) + ' руб.'
        bot.send_message(message.chat.id, text, reply_markup=get_main_menu())
...
def check_error(message):
    ...
    if code in ERROR_CODES:
        ...
        bot.send_message(message.chat.id, '❓ Код ' + code + ' не найден. Доступные коды: P0420, P0300, P0171, P0172, P0135, P0442', reply_markup=get_main_menu())
